...
class ControlDiferencial(Node):
    def __init__(self):
        ...
        # Umbrales de repulsion (+15% vs version 360 anterior)
        ...
        self.umbral_trasero = 0.35   # rango corto (original). cono trasero 95-180 deg
        ...
        self.exp_repulsion = 5.0
        # Limites angulares de los conos (radianes)
        self.ang_frontal = math.radians(20.0)   # |a|<20 -> frontal
    ...
    def scan_callback(self, msg):
        obstaculo = False
        # Repulsion frontal + lateral (alimenta el vortex)
        rep_x = 0.0
        rep_y = 0.0
        # Repulsion trasera (debil, SIN vortex): evita pegarse a la pared de atras
        # sin trabar al robot en esquinas (con peso bajo deja salir en reversa).
        rep_tras_x = 0.0
        rep_tras_y = 0.0
        ...
        for i, r in enumerate(msg.ranges):
            ...
            abs_a = abs(angle)
            # 3 conos: frontal |a|<20 ; lateral 20<=|a|<95 ; trasero |a|>=95
            es_frontal = abs_a < self.ang_frontal
            es_trasero = abs_a >= self.ang_lateral
            if es_frontal:
                umbral = self.umbral_frontal
            elif es_trasero:
                umbral = self.umbral_trasero
            else:
                umbral = self.umbral_lateral
            ...
            if r < umbral:
                obstaculo = True
                fuerza = 20.0 * ((umbral - r) / umbral)**self.exp_repulsion  # magnitud +25% (antes 16.0)
                if es_trasero:
                    # Cono trasero: repulsion debil, sin vortex
                    rep_tras_x += -fuerza * math.cos(angle) * self.peso_trasero
                    rep_tras_y += -fuerza * math.sin(angle) * self.peso_trasero
                else:
        ...
        if obstaculo:
            ...
            self.repulsion_x = (rep_x * 0.4) + (tang_x * 0.7) + rep_tras_x
            self.repulsion_y = (rep_y * 0.4) + (tang_y * 0.7) + rep_tras_y
        else:
            self.repulsion_x = 0.0
            self.repulsion_y = 0.0
    ...
    def control_loop(self):
        ...
        if self.obstaculo_cerca:
            ...
            max_rep = 0.75   # tope +25% (antes 0.60) para dejar pasar mas fuerza de repulsion
            if rep_mag > max_rep:
